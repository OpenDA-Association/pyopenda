...
class PINN(nn.Module):
    ...
    Input (x_train) is a tensor containing observations [observation(s), previous_observation(s)] or whole states [state, previous_state].
    Ouptut (y_train) is a tensor containing estimated parameter(s) [f].

    @author Aron Schouten

    """
    def __init__(self, device, layers, enkf, x_train, y_train, x_test, y_test, NN_only=False):
        """
        Constructor.
        """
        super().__init__()
    ...
    def loss_PDE(self, x):
        ...
        h_ext = torch.concat((torch.zeros(h.shape[0]).view(-1,1).to(self.device), h),-1) # Extending u and h to keep the same dimensions after taking derivative
    ...
    def loss(self, x, y):
        ...
        return (1000* loss_param if self.NN_only else 1000* loss_param + 500* loss_PDE)
